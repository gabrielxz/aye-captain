...
// 1. beyond detection, within hearing: a bearing-only rumble — and nothing else
{
  ...
  assert(snap.rumbles.length === 1 && snap.rumbles[0].cid === "s1", "rumble present");
...
// contact and NO rumble for the same emitter (never both)
{
  ...
  assert(!snap.rumbles.some((r: any) => r.cid === "s1"), "no double life: contact suppresses the rumble");
}
...
// (far beyond detection) rumbles for the enemy
{
  ...
  sim.tick();
  const snap = sim.snapshotFor("A") as any;
  assert(snap.rumbles.some((r: any) => r.cid === "s1"), "launch spike heard 400 km away");
...
// 8. decoys rumble exactly like ships (a silent 'contact' would unmask them)
{
  ...
  assert(!snap.rumbles.some((r: any) => r.cid === "s1"), "quiet mother ship inaudible");
